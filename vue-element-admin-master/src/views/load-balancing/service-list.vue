        ...
        <el-card shadow="always" style="width: 100%;">
          <span>
            <div>
              ...
              >服务（Service）提供一种抽象的方法，将运行在容器组（Pod）上的应用程序公开为网络服务。</span>
            </div>
          </span>
        </el-card>
      </el-col>
    ...
    <el-row>
      ...
      <el-col :span="4">
        <div class="header-grid-content" style="text-align: right;">
          ...
          <el-button type="primary" icon="el-icon-plus" @click="createService()">创建</el-button>
        </div>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    getService() {
      ...
      this.$store.dispatch('service/getService', this.serachInfo).then((res) => {
    ...
    createService() {
      this.$router.push({
        path: '/loadbalancing/service/create'
      })
    },
    ...
    handleEdit(namespace, name) {
      this.$store.dispatch('service/getServiceDetail', { namespace: namespace, name: name }).then((res) => {
    ...
    handleUpdate() {
      ...
      this.$store.dispatch('service/updateService', { namespace: this.detailnamespace, data: datajson }).then((res) => {
        this.$message({
          showClose: true,
          message: res.msg,
          type: 'success'
