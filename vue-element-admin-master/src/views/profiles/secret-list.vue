<template>
  <div>
          ...
          <span>
            <div>
              <svg class="header-icon" aria-hidden="true">
                <use xlink:href="#icon-a-nav_secretkey-copy" />
              </svg>
              <span
              ...
              >保密字典（Secret）是一种包含少量敏感信息的资源对象，例如密码、令牌、保密字典等，以键值对形式保存并且可以在容器组中使用。</span>
            </div>
          </span>
        </el-card>
      </el-col>
    ...
    <el-row>
      ...
      <el-col :span="14">
        <div class="header-grid-content">
          <el-input
            ...
            clearable
            @clear="getSecret()"
            @keyup.native.enter="getSecret()"
          >
            <template #prepend>
              <el-button icon="el-icon-search" @click="getSecret()" />
            </template>
          </el-input>
        </div>
      </el-col>
      <el-col :span="4">
        <div class="header-grid-content" style="text-align: right;">
          <el-button type="info" icon="el-icon-refresh" round @click="getSecret()" />
    ...
    <div class="table-bg-purple">
      <el-table :data="secretItem" :header-cell-style="{ background: '#e6e7e9' }" size="small" empty-text="抱歉，暂无数据">
        ...
        <el-table-column label="字典数" prop="count_data" width="70" />
...
export default {
  ...
  methods: {
    getSecret() {
      ...
      this.$store.dispatch('secret/getSecret', this.serachInfo).then((res) => {
    ...
    handleEdit(namespace, name) {
      this.$store.dispatch('secret/getSecretDetail', { namespace: namespace, name: name }).then((res) => {
    ...
    handleDelete(namespace, name) {
      this.$confirm(`是否删除实例 ${name}`, '提示', {
        ...
        this.$store.dispatch('secret/deleteSecret', {
          namespace: namespace,
          secret_name: name
        }).then((res) => {
          this.$message({
    ...
    handleUpdate() {
      ...
      this.$store.dispatch('secret/updateSecret', { namespace: this.detailnamespace, data: datajson }).then((res) => {
        this.$message({
          showClose: true,
          message: res.msg,
          type: 'success'
