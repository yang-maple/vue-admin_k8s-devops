          ...
          <span>
            <div>
              <svg class="header-icon" aria-hidden="true">
                <use xlink:href="#icon-statefulset" />
              </svg>
              <span
    ...
    <el-row>
      ...
      <el-col :span="14">
        <div class="header-grid-content">
          <el-input
            ...
            @keyup.native.enter="getStatefulset()"
          >
            <template #prepend>
              <el-button icon="el-icon-search" @click="getStatefulset()" />
            </template>
          </el-input>
        </div>
      </el-col>
      <el-col :span="4">
        <div class="header-grid-content" style="text-align: right;">
          <el-button type="info" icon="el-icon-refresh" round @click="getStatefulset()" />
        </div>
      </el-col>
    </el-row>

    <div class="table-bg-purple">
      <el-table :data="statefulItem" :header-cell-style="{ background: '#e6e7e9' }" size="small" empty-text="抱歉，暂无数据">
...
export default {
  ...
  methods: {
    getStatefulset() {
      ...
      this.$store.dispatch('statefulset/getStatefulset', this.serachInfo).then((res) => {
    ...
    handleEdit(namespace, name) {
      this.$store.dispatch('statefulset/getStatefulsetDetail', { namespace: namespace, name: name }).then((res) => {
    ...
    handleReplica(namespace, name, replicas) {
      this.$store.dispatch('statefulset/modifyStatefulset', {
    ...
    handleUpdate() {
      ...
      this.$store.dispatch('statefulset/updateStatefulset', { namespace: this.detailnamespace, data: datajson }).then((res) => {
        this.$message({
          showClose: true,
          message: res.msg,
          type: 'success'
